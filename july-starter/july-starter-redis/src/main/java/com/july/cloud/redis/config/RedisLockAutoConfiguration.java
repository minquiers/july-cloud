...
import com.july.cloud.redis.lock.RedisLockAspect;
import com.july.cloud.redis.lock.RedisLockClient;
import com.july.cloud.redis.lock.RedisLockClientImpl;
import com.july.cloud.redis.lock.RedisLockProperties;
...
@ConditionalOnProperty(value = "redis.lock.enabled", havingValue = "true")
public class RedisLockAutoConfiguration {
    ...
    private static Config singleConfig(RedisLockProperties properties) {
        ...
        if (properties.isSsl()) {
            serversConfig.setSslEnableEndpointIdentification(false);
            serversConfig.setAddress(properties.getAddress().replace("redis", "rediss"));
        } else {
